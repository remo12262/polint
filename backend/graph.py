...
DB_PATH = os.environ.get("DB_PATH", "polint.db")
...
class PolintDB:
    ...
    async def get_nodes(self, domain=None) -> List[Dict]:
        async with aiosqlite.connect(DB_PATH) as db:
            ...
            cursor = await db.execute("SELECT * FROM nodes ORDER BY influence_score DESC")
            return [dict(r) for r in await cursor.fetchall()]

    async def get_edges(self, domain=None) -> List[Dict]:
        async with aiosqlite.connect(DB_PATH) as db:
            db.row_factory = aiosqlite.Row
            cursor = await db.execute("SELECT * FROM edges ORDER BY hidden_score DESC")
            return [dict(r) for r in await cursor.fetchall()]
    ...
    async def get_influence_path(self, node_id: str) -> List[Dict]:
        """Get 2-hop influence path for a node."""
        async with aiosqlite.connect(DB_PATH) as db:
            db.row_factory = aiosqlite.Row
            cursor = await db.execute("""
    ...
    async def get_predictions(self) -> List[Dict]:
        async with aiosqlite.connect(DB_PATH) as db:
            db.row_factory = aiosqlite.Row
            cursor = await db.execute("SELECT * FROM predictions ORDER BY confidence DESC, created_at DESC LIMIT 30")
            return [dict(r) for r in await cursor.fetchall()]

    async def get_hidden_networks(self) -> List[Dict]:
        async with aiosqlite.connect(DB_PATH) as db:
            db.row_factory = aiosqlite.Row
            cursor = await db.execute("SELECT * FROM hidden_networks ORDER BY opacity_score DESC")
            return [dict(r) for r in await cursor.fetchall()]
    ...
    async def get_alerts(self, severity=None) -> List[Dict]:
        async with aiosqlite.connect(DB_PATH) as db:
            db.row_factory = aiosqlite.Row
            if severity:
                cursor = await db.execute("SELECT * FROM alerts WHERE severity=? ORDER BY confidence DESC, created_at DESC", (severity,))
            else:
                cursor = await db.execute("SELECT * FROM alerts ORDER BY confidence DESC, created_at DESC LIMIT 50")
            return [dict(r) for r in await cursor.fetchall()]
    ...
    async def get_stats(self) -> Dict:
        async with aiosqlite.connect(DB_PATH) as db:
            ...
            p = (await (await db.execute("SELECT COUNT(*) FROM predictions")).fetchone())[0]
            a = (await (await db.execute("SELECT COUNT(*) FROM alerts WHERE is_read=0")).fetchone())[0]
            hn = (await (await db.execute("SELECT COUNT(*) FROM hidden_networks")).fetchone())[0]
            hi = (await (await db.execute("SELECT COUNT(*) FROM nodes WHERE hidden_score > 60")).fetchone())[0]
            return {"nodes": n, "edges": e, "predictions": p, "unread_alerts": a, "hidden_networks": hn, "high_opacity_actors": hi}
    ...
    async def upsert_entities(self, entities: List[Dict]):
        ...
        async with aiosqlite.connect(DB_PATH) as db:
            for e in entities:
                await db.execute("""
                    INSERT INTO nodes (id,label,type,domain,country,description,influence_score,hidden_score,created_at,updated_at)
                    VALUES (?,?,?,?,?,?,?,?,?,?)
                    ON CONFLICT(id) DO UPDATE SET
                        influence_score = MAX(influence_score, excluded.influence_score),
                        hidden_score = MAX(hidden_score, excluded.hidden_score),
                        updated_at = excluded.updated_at
                """, (e.get("id"),e.get("label"),e.get("type","Organization"),"political",
                      e.get("country"),e.get("description"),e.get("influence_score",0),
                      e.get("hidden_score",0),now,now))
            await db.commit()
    ...
    async def upsert_relations(self, relations: List[Dict]):
        ...
        async with aiosqlite.connect(DB_PATH) as db:
            for r in relations:
                ...
                await db.execute("""
                    INSERT INTO edges (id,source,target,type,fact,influence_score,hidden_score,source_doc,date,created_at)
                    VALUES (?,?,?,?,?,?,?,?,?,?)
                    ON CONFLICT(id) DO UPDATE SET
                        hidden_score = MAX(hidden_score, excluded.hidden_score)
    ...
    async def upsert_hidden_networks(self, networks: List[Dict]):
        ...
        async with aiosqlite.connect(DB_PATH) as db:
            for hn in networks:
                await db.execute("""
                    INSERT OR REPLACE INTO hidden_networks
